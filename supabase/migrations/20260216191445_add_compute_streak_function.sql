...
-- Compute streak for a user (consecutive days with usage, ending today or yesterday)
CREATE OR REPLACE FUNCTION compute_streak(target_user_id UUID)
RETURNS INTEGER AS $$
DECLARE
...
BEGIN
  ...
  IF NOT EXISTS(SELECT 1 FROM daily_usage WHERE user_id = target_user_id AND date = CURRENT_DATE) THEN
